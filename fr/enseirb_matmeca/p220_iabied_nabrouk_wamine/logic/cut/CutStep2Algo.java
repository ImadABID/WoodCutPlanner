...
public class CutStep2Algo implements CutAlgos {
    ...
    public List<Cut> optimiseCuts(List<Board> boards, List<Panel> panels)throws ParserConfigurationException, IOException, SAXException, ParseException{
        ...
        for(int i = 0; i<boards.size(); i++) {
            ...
            board_length=board_rect.getLenght();
            ...
            d_client = boards.get(i).getCritical_date().date;
            ...
            for(int j=0;j< panels.size();j++){
                ...
                System.out.println("values " + nbr_pieces.value);
                d_supplier = panels.get(j).getCritical_date().date;
                deliv_date = new SimpleDateFormat("dd.MM.yy").parse(d_supplier);
                if (nbr_pieces.value >0 && deliv_date.before(recep_date) && panel_length>=board_length && panel_width>=board_width){
